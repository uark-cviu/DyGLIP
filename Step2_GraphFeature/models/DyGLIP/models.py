...
class DyGLIP(Model):
    ...
    def __init__(self, placeholders, num_features, num_features_nonzero, degrees, eval=False, **kwargs):
        super(DyGLIP, self).__init__(**kwargs)
        self.attn_wts_all = []
        ...
        if FLAGS.window < 0:
            ...
            self.num_time_steps = min(
                len(placeholders['features']), FLAGS.window + 1)
        self.num_time_steps_train = self.num_time_steps - 1
        self.num_features = num_features
        self.num_features_nonzero = num_features_nonzero
        self.degrees = degrees
        self.num_features = num_features
        self.structural_head_config = list(map(
            int, FLAGS.structural_head_config.split(",")))
        self.structural_layer_config = list(map(
            int, FLAGS.structural_layer_config.split(",")))
        self.temporal_head_config = list(map(
            int, FLAGS.temporal_head_config.split(",")))
        self.temporal_layer_config = list(map(
            int, FLAGS.temporal_layer_config.split(",")))
        self._build()
    ...
    def _build(self):
        ...
        # Build actual model.
        self.final_output_embeddings, self.structural_output_embeddings = self.build_net(self.structural_head_config, self.structural_layer_config,
    ...
    def build_net(self, attn_head_config, attn_layer_config, temporal_head_config, temporal_layer_config,
        ...
        for i in range(0, len(attn_layer_config)):
            if i > 0:
                ...
                sparse_inputs = False
            structural_layer = StructuralAttentionLayer(input_dim=input_dim,
                                                     ...
                                                     residual=FLAGS.use_residual)
            self.structural_attention_layers.append(structural_layer)
        ...
        # List of t feature matrices. [N x F]
        input_list = self.placeholders['features']
        for layer in self.structural_attention_layers:
            attn_outputs = []
            inputs = []
            for t in range(0, self.num_time_steps):
                out, inp_seq = layer([input_list[t], adjs[t]])
                attn_outputs.append(out)  # A list of [1x Ni x F]
                inputs.append(inp_seq)
            input_list = list(attn_outputs)

        structural_outputs_debug = inputs
        # 4: Pack embeddings across snapshots.
        for t in range(0, self.num_time_steps):
            zero_padding = tf.zeros(
                [1, tf.shape(attn_outputs[-1])[1] - tf.shape(attn_outputs[t])[1], attn_layer_config[-1]])
            attn_outputs[t] = tf.concat(
                [attn_outputs[t], zero_padding], axis=1)

        structural_outputs = tf.transpose(
            tf.concat(attn_outputs, axis=0), [1, 0, 2])  # [N, T, F]
        ...
        return outputs, structural_outputs_debug
    ...
    def _loss(self):
        ...
        for t in range(self.num_time_steps_train - num_time_steps_train, self.num_time_steps_train):
            output_embeds_t = tf.nn.embedding_lookup(
                tf.transpose(self.final_output_embeddings, [1, 0, 2]), t)
            self.inputs1 = tf.nn.embedding_lookup(
                output_embeds_t, self.placeholders['node_1'][t])
            self.inputs2 = tf.nn.embedding_lookup(
                output_embeds_t, self.placeholders['node_2'][t])
            pos_score = tf.reduce_sum(self.inputs1 * self.inputs2, axis=1)
            self.neg_samples = tf.nn.embedding_lookup(
                output_embeds_t, self.proximity_neg_samples[t])
            self.neg_samples_2 = tf.nn.embedding_lookup(
                output_embeds_t, self.placeholders['node_3'][t])
            neg_score = (-1.0) * tf.matmul(self.inputs1, tf.transpose(self.neg_samples_2))
            pos_ent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(pos_score), logits=pos_score)
            neg_ent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(neg_score), logits=neg_score)
            self.num_pos_samples += tf.shape(self.inputs1)[0]
            self.num_neg_samples += tf.shape(self.neg_samples_2)[0] # + tf.shape(self.neg_samples_2)[0]
            self.graph_loss += tf.reduce_mean(pos_ent) + \
                FLAGS.neg_weight * tf.reduce_mean(neg_ent)
